...
* `indi://mcp-server/guiding` (design only — INDIMCP-26, not yet implemented) — a planned fourth member of this family carrying guiding events translated from PHD2's Event Monitoring API (`guidingStep`, `guidingSettling`, `guidingSettleDone`, `guidingStarLost`, `guidingAlert`, ...); PHD2's own connection lifecycle publishes to `indi://mcp-server/connection` with `target="phd2"`, reusing the mechanism above rather than adding a separate one. See [Guiding.md](Guiding.md) for the full design.
...
**`frames`'s `action="list"`** (INDIMCP-120, replacing the old standalone `list_frames` tool) — query frame metadata with optional filters, returning `frame_id`s and everything else in the `frames` table except the file path (an internal server-side detail, not exposed to the client). `transferred` is tri-state (omitted/`null` = every frame, `true` = only already-transferred ones, `false` = only ones still waiting to be retrieved) rather than a one-directional `transferredOnly` flag, since knowing what's still pending is just as useful as seeing what's done — e.g. before deciding it's safe to run `manage_frame`'s `action="purge"` (below). Returns a bare array, not a `kind`-tagged envelope — matching every other list-shaped tool in this codebase (`list_config`, `list_indi_drivers`, ...), none of which wrap their results either; `kind` tags are for single event/status-shaped objects, not list results:
...
[
  {
    ...
    "transferredAt": null,
    "downloadUrl": "/frames/frame-0001",
    "issues": []
  }
]
```

**`frames`'s `action="get"`** (replacing the old standalone `get_frame_metadata` tool) — the same shape for a single `frameId`, useful once a client already knows the id (e.g. from a `frames`'s `action="list"`, `runId=...` call made after a `scriptCompleted` event).

**Frame content is downloaded over plain HTTP, not read as an MCP resource** (INDIMCP-89, superseding the original `frame://{frame_id}` resource design below). Each frame's `downloadUrl` above is a path — `/frames/{frameId}` — not an absolute URL, resolved by the client against the same host:port it already used to connect via MCP (INDIMCP-96). It points at a plain Starlette route registered via FastMCP's `custom_route` (see `server.py`'s `download_frame`), living on the same host/port as the MCP endpoint itself but entirely outside the MCP session/protocol — that shared registration is exactly what makes the path safe to resolve relative to the client's own connection endpoint rather than requiring the server to name a host itself. The handler streams the file straight off disk (`FileResponse`); no base64 encoding, no full in-memory buffering, no single-message size ceiling. `downloadUrl` is `null` when the server has no HTTP listener to build one from (the `stdio` transport, e.g. local testing) — there's nothing to point at in that case.

Originally, frame content was exposed as an MCP resource instead: each frame got a `frame://{frame_id}` URI, read via the standard `resources/read` request, reusing MCP's existing binary content handling (a base64 `blob` resource content) rather than inventing a separate transfer mechanism. This was deliberately left open at the time ("deferred until real frame sizes from actual hardware are known rather than solved speculatively now") — MCP's resource-read mechanism returns the whole content in one JSON-RPC response, with no native chunked/range read to fall back on for a large one. Real frame sizes turned out to matter: an 18MB raw (~23MB base64-encoded) dark frame disconnected a real MCP client reading it this way. Since that's a protocol-level ceiling (any MCP client hits it eventually, just at a different size threshold depending on its own timeout/memory limits, not something specific to one client implementation), the fix moves the bytes outside the MCP protocol entirely rather than inventing an offset/length chunking convention on top of it. `frame://{frame_id}` itself has been removed — there was no safe use case left for it once it broke on real frame sizes, and keeping two ways to fetch the same bytes would mean two code paths to maintain and test for one that's known to fail. The new endpoint is unauthenticated, same as every MCP tool/resource this server already exposes over `streamable-http` (see Deployment.md's Hardening notes) — this doesn't introduce a new class of exposure, just a new URL path with the same trust model. `downloadUrl` originally carried an absolute URL built from `socket.gethostname()`, which isn't reliably resolvable from every client's network (mDNS can be disabled or blocked across subnets) even though the exact same server is already reachable at whatever host the client used to connect via MCP in the first place; INDIMCP-96 replaced it with the host-relative path above, removing the hostname-resolution problem by construction.

**Explicit transfer confirmation, not read-implies-received.** `transferred_at` is *not* set just because the server sent the bytes — a network drop mid-transfer shouldn't be recorded as a successful transfer. Instead, the client calls `manage_frame`'s `action="confirm_transfer"` with the `frameId` once it has verified the frame is safely saved locally; only that sets `transferred_at`. This follows the same "don't trust delivery, wait for acknowledgement" principle already applied to the event log and script status. `checksumSha256` (INDIMCP-95) lets that verification check the downloaded bytes' actual content, not just their length against `sizeBytes` — a truncated-but-coincidentally-same-length transfer would pass a size check but fail a hash comparison. `checksumSha256` is `null` for a frame captured before checksum support existed (its row predates the `checksum_sha256` column and was carried forward by a schema migration with nothing to hash) — always populated for every frame captured since. When it's `null`, `frames`'s `action="list"`/`"get"` also add a `frameChecksumMissing` `WARNING` to that frame's `issues` array (the same `kind`/`severity`-tagged `Issue` shape `run_script` status responses use for non-fatal conditions), so a client can surface *why* the checksum is missing rather than treating a `null` as unexplained.

## Deleting frames

The INDI Device's own storage is limited, so frames need cleaning up once the Client Computer has its own copy — but never automatically: the server itself never deletes a frame on its own initiative, only in direct response to one of these two client-initiated actions.

**`manage_frame`'s `action="delete"`** (INDIMCP-120, replacing the old standalone `delete_frame` tool) — deletes one frame's file and metadata row, identified by `frameId`. Refuses (raises an error) if that frame's `transferredAt` isn't set yet, unless the caller explicitly passes `requireTransferred: false` — deleting the only copy of a frame the Client Computer never actually confirmed receiving would be data loss, so this is safe by default rather than trusting every caller to check first.
