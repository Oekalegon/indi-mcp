...
`ConnectionMade`/`ConnectionLost` used to be listed here too, as a deliberate exclusion; INDIMCP-57
implemented it instead — see [Connection-lifecycle envelope](#connection-lifecycle-envelope) below.
...
Implemented by INDIMCP-57; published to `indi://mcp-server/connection` and the durable event
log, per [Design.md § Event streams](Design.md#event-streams). Distinct from both the
messaging-layer and scripting-layer envelopes above — it carries no `type`/`name`/`elements`
(there's no property involved) and no `runId`/`rigId` (there's no script run involved) — so it
gets its own minimal shape rather than force-fitting into either.

| Field | Type | Description |
|---|---|---|
| `kind` | string | `"connectionMade"` or `"connectionLost"`. |
| `target` | string | Which connection this event is about: `"server"` (this MCP server's own TCP link to `indiserver`), `"indiserver"` (the `indiserver` process itself), or an INDI driver's catalog label (e.g. `"CCD Simulator"`, an individual driver process). |
| `message` | string \| null | Optional human-readable detail (e.g. `"indiserver started on port 7624"`). `null` for `target="server"` events, which are sourced directly from `indipyclient`'s own `ConnectionMade`/`ConnectionLost` events and carry no message text on the wire. |
| `timestamp` | string | ISO 8601, UTC. |

Three producers, one shape:

* **`target="server"`** — `indi_messaging._MessagingClient.rxevent` handles `indipyclient`'s
  local `ConnectionMade`/`ConnectionLost` events directly (ahead of the `_to_indi_event`
  fallthrough documented in [Scope: other INDI client events](#scope-other-indi-client-events)
  above). Fires on every (re)connect and disconnect of this server's own TCP link to
  `indiserver` — including `indipyclient`'s internal 5-second reconnect retry loop, so a
  sustained outage on `indiserver`'s side surfaces as a repeating pair, not a single event.
* **`target="indiserver"`** — `indi_server.start_server`/`stop_server` publish when they
  actually observe the managed `indiserver` process's running state change (start/stop
  succeeding, not merely being requested).
* **`target=<driver label>`** — `indi_driver.start_driver`/`stop_driver` publish on a
  successful start/stop of that specific driver process.

Unlike `publish_message_event`'s de-duplication of consecutive identical wire updates
(INDIMCP-88), `publish_connection_event` never suppresses repeats — a repeated
`connectionLost` for the same `target` (e.g. a sustained reconnect loop) is itself meaningful,
not noise.
...
```json
{
  "kind": "connectionLost",
  "target": "indiserver",
  "message": "indiserver stopped on port 7624",
  "timestamp": "2026-08-18T16:12:00Z"
}
```
