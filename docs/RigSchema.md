...
| `trainId` | string | no | Groups this component with others mounted on the same imaging train (e.g. `"ota1"`) — see above. If given, must be non-empty. Two components with the same `trainId` may not share a `role` (a rig where they do fails to load). |
